...
class Medida():
	...
	def __init__(self, medida, incerteza=None, algarismos=0, formatacao = (lambda m: helper.formatar_com_erro(m.media, m.incerteza))):
		...
		if isinstance(medida, float) or isinstance(medida,int): #Se é número, checar possíveis casos

			#Processar a incerteza
			if isinstance(incerteza, float) or isinstance(incerteza, int):
				self.incerteza = float(incerteza)

			elif isinstance(incerteza, iterable): #Se é lista, calcular a incerteza combinada
		...
		elif helper.iteravel(medida): #Se é lista, calcular incerteza do desvio-padrão populacional
			...
			for md in medida:
				u_ph += math.pow(self.media - md, 2) #Somar diferença entre média e medida ao quadrado

			self.incerteza = math.pow(u_ph/(n*(n-1)), 1/2)

		else:
			raise Exception("Inicialização incorreta da classe Medida em <medida>")

		if not isinstance(algarismos, int): #Checar se o argumento "algarismos" foi bem definido (se não for inteiro)
			raise Exception("Inicialização incorreta da classe Medida em <algarismos> (<algarismos> não inteiro)")

		if algarismos < 0: #Checar se o argumento "algarismos" foi bem definido (se for inteiro)
			raise Exception("Inicialização incorreta da classe Medida em <algarismos> (<algarismos> < 0)")
...
class RegressaoLinear:
	...
	def __init__(self, dados=[]):
		...
		if not helper.iteravel(dados):
			raise Exception("Inicialização incorreta da classe RegressaoLinear")
	...
	def calcular(self, inverter_eixos = False):
		...
		if inverter_eixos:
			...
			u_ph = self.dados[0].incerteza #Placeholder de incerteza para checagem de caso de incertezas iguais
			...
			self._resultados = self._mmq(dados_y, dados_x, (self.dados[0][0].incerteza if u_iguais else [d[0].incerteza for d in self.dados]))

		else:
			u_ph = self.dados[0][1].incerteza #Placeholder de incerteza para checagem de caso de incertezas iguais
			...
			#Instanciar lista de u's apenas se <u_iguais> for falso
			self._resultados = self._mmq(dados_x, dados_y, (self.dados[0][1].incerteza if u_iguais else [d[1].incerteza for d in self.dados]))
	...
	def coeficientes_invertidos(self):
		...
		if not self._resultados:
			raise Exception("A regressão linear não tem resultados para que sejam calculados seus coeficientes invertidos")

		a_invertido = 1/self._resultados['a'].media
		ua_invertido =  self._resultados['a'].incerteza/math.pow(self._resultados['a'].media, 2)

		b_invertido = -self._resultados['b'].media/self._resultados['a'].media

		#Cálculo otimizado de ub_invertido
		ub_invertido = math.pow(math.pow(self._resultados['b'].incerteza,2) + math.pow(self._resultados['a'].incerteza*self._resultados['b'].media/self._resultados['a'].media,2),1/2)/abs(self._resultados['a'].media)
